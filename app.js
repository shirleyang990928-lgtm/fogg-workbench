...
async function loadUserDataFromCloud(){
  ...
  if(data){
    ...
    // 老师资料/笔记：云端有这个字段（且有数据）就用云端，否则保留本地（字段还没建时）
    if(Array.isArray(data.teachers)&&data.teachers.length){
      teachersData=data.teachers.map(normalizeTeacher);
      localStorage.setItem("teachersDataLocal",JSON.stringify(teachersData));
    }
...
// 老师资料 + 沟通笔记（v20260614i）：本地用专用 key 存（不在 STORAGE_KEYS，登录时不会被清），
// 云端用单独 upsert 同步（数据库加了 teachers 字段才成功，没加也不影响其他同步）
let teachersData = loadCollection("teachersDataLocal", [], normalizeTeacher);
...
function renderManage(){setHead("\u7ba1\u7406","\u5206\u6b65\u6574\u7406\u8bdd\u672f\u3001\u8bfe\u7a0b\u3001\u56de\u6536\u7ad9\u548c\u5907\u4efd","");byId("tabs").innerHTML=tabs([{value:"home",label:"\u5165\u53e3"},{value:"workload",label:"\u672c\u5468\u5de5\u4f5c\u91cf"},{value:"stickers",label:"\u7ba1\u7406\u8bdd\u672f"},{value:"classes",label:"\u7ba1\u7406\u8bfe\u7a0b"},{value:"trash",label:"\u56de\u6536\u7ad9"},{value:"backup",label:"\u5907\u4efd"}],manageMode,"manage");if(manageMode==="home")renderManageHome();if(manageMode==="workload")renderWorkloadPage();if(manageMode==="stickers")renderStickerManage();if(manageMode==="classes")renderClassManage();if(manageMode==="trash")renderTrash();if(manageMode==="backup")renderBackup();bindManageEvents();}
...
function perClassWorkload(since,until){
  return overviewCourses().map(c=>{
    let lessons=0,ungraded=0;
    (Array.isArray(c.classRecords)?c.classRecords:[]).forEach(rec=>{
      const d=String(rec.date||"");if(d<since||d>until)return;
      const hw=rec.homework||{};
      if(Object.keys(rec.attendance||{}).length||homeworkAssigned(hw)||rec.notes||rec.materials)lessons++;
      if(homeworkAssigned(hw))Object.keys(hw.entries||{}).forEach(n=>{if((hw.entries[n]||{}).state==="已交")ungraded++;});
    });
    const names=new Set((c.students||[]).map(s=>(s.name||"").trim()).filter(Boolean));
    let followups=0,makeups=0;
    studentsData.forEach(p=>{if(!names.has(p.name))return;
      (p.followups||[]).forEach(f=>{if(String(f.date)>=since&&String(f.date)<=until)followups++;});
      (p.makeups||[]).forEach(m=>{const md=m.madeAt?dateKey(new Date(m.madeAt)):m.date;if(md>=since&&md<=until&&m.classId===c.id)makeups++;});
    });
    const owed=Object.keys(homeworkOwedInRange(since,until,"").owed).filter(n=>names.has(n)).length;
    return {c,lessons,ungraded,followups,makeups,owed};
  }).filter(x=>x.lessons||x.followups||x.makeups||x.owed)
    .sort((a,b)=>(b.lessons+b.followups+b.makeups)-(a.lessons+a.followups+a.makeups));
}
...
function renderWorkloadPage(){
  ...
  const since=workloadRange?workloadRange.from:def[0];
  const until=workloadRange?workloadRange.to:def[1];
  const rt=rangeText(since,until);
  const s=workloadStats(since,until);
  const perClass=perClassWorkload(since,until);
  const reportText=workloadReportText(since,until,s,perClass);
...
function globalSearchResultsHtml(){
  const kw=globalSearchQ.trim().toLowerCase();
  if(!kw)return '<p class="empty gs-empty">输入名字，搜学生、课程、老师，点结果直接跳过去。</p>';
  const stu=studentRoster().filter(r=>r.name.toLowerCase().includes(kw)||(r.profile&&[r.profile.school,r.profile.city,r.profile.parentName,r.profile.grade].filter(Boolean).join(" ").toLowerCase().includes(kw))).slice(0,10);
  const courses=overviewCourses().filter(c=>(c.className||"").toLowerCase().includes(kw)||(c.teacher||"").toLowerCase().includes(kw)).slice(0,10);
  const teachers=[...new Set(overviewCourses().map(c=>(c.teacher||"").trim()).filter(Boolean))].filter(t=>t.toLowerCase().includes(kw)).slice(0,10);
  if(!stu.length&&!courses.length&&!teachers.length)return '<p class="empty gs-empty">没找到匹配的学生 / 课程 / 老师。</p>';
  const grp=(title,items)=>items.length?`<div class="gs-group"><h5 class="gs-group-title">${title} · ${items.length}</h5><div class="gs-items">${items}</div></div>`:"";
  const stuItems=stu.map(r=>`<button class="gs-item" data-gs-student="${safeAttr(r.name)}" type="button"><b>${esc(r.name)}</b><small>${r.classes.length} 门课${r.profile&&r.profile.grade?" · "+esc(r.profile.grade):""}</small></button>`).join("");
  const courseItems=courses.map(c=>`<button class="gs-item" data-gs-course="${safeAttr(c.id)}" type="button"><b>${esc(c.className)}</b><small>${esc(c.weekday)} ${esc(formatTimeCN(c.time))} · ${esc(c.teacher||"未填")}</small></button>`).join("");
...
function refreshGlobalSearch(){
  ...
  if(!box)return;
  box.innerHTML=globalSearchQ.trim()?globalSearchResultsHtml():"";
  bindGlobalSearchResults();
}
function bindGlobalSearchResults(){
  document.querySelectorAll("[data-gs-student]").forEach(b=>b.addEventListener("click",()=>{editingStudentName=b.dataset.gsStudent;studentDetailEditing=false;followupFormFor="";stuFuOpen=false;stuTlOpen=false;view="students";render();}));
  document.querySelectorAll("[data-gs-course]").forEach(b=>b.addEventListener("click",()=>openCourseHome(b.dataset.gsCourse)));
  document.querySelectorAll("[data-gs-teacher]").forEach(b=>b.addEventListener("click",()=>openTeacher(b.dataset.gsTeacher)));
}
...
/* 老师资料/笔记（v20260614i）：name 为主键，logs=[{id,date,text}] */
function normalizeTeacher(t){
  ...
  return {
    ...
    // 学生对老师的反馈（v20260615a）：[{id,date,student,text}]
    feedbacks:Array.isArray(t.feedbacks)?t.feedbacks:[],
    updatedAt:t.updatedAt||new Date().toISOString()
  };
}
function getTeacher(name){
  const k=String(name||"").trim();
  let t=teachersData.find(x=>x.name===k);
  if(!t){t=normalizeTeacher({name:k});teachersData.push(t);}
  if(!Array.isArray(t.logs))t.logs=[];
  if(!Array.isArray(t.feedbacks))t.feedbacks=[];
  return t;
}
function saveTeachers(){
  if(adminViewEmail)return;
  localStorage.setItem("teachersDataLocal",JSON.stringify(teachersData));
  // 云端：单独 upsert，只动 teachers 列；没建这个字段就静默跳过，不影响其他数据同步
  if(currentUser){
    try{
      sb.from('user_data').upsert({user_email:currentUser.email,teachers:teachersData},{onConflict:'user_email'})
        .then(function(r){if(r&&r.error)console.warn('teachers 云端同步待加字段：',r.error.message);});
    }catch(e){/* 本地已保存，忽略 */}
  }
}
...
function studentDetailHtml(){
  ...
  <div class="student-detail-extra">
    ...
    ${studentTeacherFeedbackHtml(editingStudentName)}
...
function renderStudents(){
  ...
  if(back)back.addEventListener("click",()=>{editingStudentName=null;followupFormFor="";followupEditId="";stuFuOpen=false;stuTlOpen=false;render();});
  // TA 对老师的反馈 → 跳老师面板（联动）
...
function bindStudentPicks(){
  document.querySelectorAll("[data-pick-student]").forEach(b=>b.addEventListener("click",()=>{editingStudentName=b.dataset.pickStudent;studentDetailEditing=false;stuTimelineCourse="all";stuTimelineKind="all";studentLinkPickerOpen=false;followupFormFor="";followupEditId="";stuFuOpen=false;stuTlOpen=false;render();}));
}
...
let followupEditId="";     // 正在编辑的跟进记录 id（空=新增）
// 老师页筛选（镜像课程总览）
...
function studentFeedbacksFor(name){
  const key=String(name||"").trim();const out=[];
  (typeof teachersData!=="undefined"?teachersData:[]).forEach(t=>{
    (t.feedbacks||[]).forEach(f=>{if((f.student||"").trim()===key)out.push({teacher:t.name,fb:f});});
  });
  return out.sort((a,b)=>String(b.fb.date).localeCompare(String(a.fb.date)));
}
...
function updateFollowup(name,id,patch){
  const p=studentsData.find(x=>(x.name||"").trim()===String(name).trim());
  if(!p||!Array.isArray(p.followups))return;
  const f=p.followups.find(x=>x.id===id);if(!f)return;
  Object.assign(f,patch);p.updatedAt=new Date().toISOString();
  saveStudents();
}
...
function followupFormHtml(name,edit){
  const f=edit||{};
  const dv=f.date||dateKey(new Date());
  const opt=s=>`<option value="${safeAttr(s)}"${f.status===s?" selected":""}>${s}</option>`;
  return `<div class="fu-form" data-bb-form="${safeAttr(name)}">
    <div class="fu-form-grid">
      <label class="fu-field"><span>跟进日期</span><input type="date" class="bb-f-date" value="${safeAttr(dv)}"></label>
      <label class="fu-field"><span>跟进后状态</span><select class="bb-f-status">${FOLLOWUP_STATUS.map(opt).join("")}</select></label>
    </div>
    <label class="fu-field"><span>我做了什么 / 用了什么话术</span><textarea class="bb-f-action" rows="2" placeholder="例：私信家长说明孩子连续两次没交，建议每天固定一个写作业时间…">${esc(f.action||"")}</textarea></label>
    <label class="fu-field"><span>家长怎么回应</span><textarea class="bb-f-reply" rows="2" placeholder="例：家长说最近忙，会盯着孩子按时交">${esc(f.parentReply||"")}</textarea></label>
    <label class="fu-field"><span>补充备注（选填 · 孩子后面有没有交，系统会自动算）</span><input class="bb-f-result" value="${safeAttr(f.result||"")}" placeholder="例：孩子说会改，这里可写点别的观察"></label>
    <div class="fu-form-ops"><button class="btn primary bb-f-save" data-bb-save="${safeAttr(name)}" type="button">${edit?"保存修改":"保存这次跟进"}</button><button class="btn ghost bb-f-cancel" type="button">取消</button></div>
  </div>`;
}
...
function bindFollowupControls(){
  ...
  document.querySelectorAll("[data-bb-log]").forEach(b=>b.addEventListener("click",()=>{
    const n=b.dataset.bbLog;followupEditId="";followupFormFor=(followupFormFor===n)?"":n;if(followupFormFor)stuFuOpen=true;render();
  }));
  // 点某条跟进的"✎"→ 内联编辑（预填该条）
  document.querySelectorAll("[data-fu-edit]").forEach(b=>b.addEventListener("click",()=>{
    const [,id]=b.dataset.fuEdit.split("|");
    followupEditId=id;followupFormFor="";stuFuOpen=true;render();
  }));
  document.querySelectorAll(".bb-f-cancel").forEach(b=>b.addEventListener("click",()=>{followupFormFor="";followupEditId="";render();}));
  document.querySelectorAll("[data-bb-save]").forEach(b=>b.addEventListener("click",()=>{
    ...
    if(followupEditId){
      updateFollowup(name,followupEditId,{date,action,parentReply,result,status});
      followupEditId="";showToast("已更新这条跟进");
    }else{
...
function studentFollowupSectionHtml(name){
  ...
  const editing=!!followupEditId&&studentFollowups(name).some(f=>f.id===followupEditId);
  const formOpen=(followupFormFor===name)&&!editing; // 编辑某条时不另开新增表单
  const open=stuFuOpen||formOpen||editing; // 记录/编辑跟进时强制展开
...
/* 学生档案里"TA 对老师的反馈"（和老师页联动，点一条跳到那位老师面板去改）（v20260615b） */
function studentTeacherFeedbackHtml(name){
  const list=studentFeedbacksFor(name);
  if(!list.length)return "";
  return `<section class="fu-card stu-tfb-card">
    <div class="fu-card-head"><h4>⭐ TA 对老师的反馈</h4><i class="fu-badge fu-badge-ok">${list.length} 条</i></div>
    <div class="stu-tfb-list">${list.map(x=>`<button class="stu-tfb-item" data-open-teacher="${safeAttr(x.teacher)}" type="button"><b>${esc(x.teacher)}</b><span>${esc(x.fb.text)}</span><small>${esc(formatDateShort(x.fb.date))} · 点开去老师页改 ›</small></button>`).join("")}</div>
  </section>`;
}
...
function renderTeachers(){
  ...
  const tableRows=summary.map(t=>`<button class="tt-row${t.gradeRate!==null&&t.gradeRate<60?' tt-grade-low':''}" data-teacher-pick="${safeAttr(t.teacher)}" type="button">
    <span class="tt-name"><b>${esc(t.teacher)}</b></span>
    <span class="tt-count">${t.courseCount} 班 · ${t.studentCount} 人</span>
...
function openTeacher(name){
  ...
  teacherPanelSel=t;teacherEditing=false;teacherLogEditId="";teacherFbEditId="";teacherType="all";teacherTerm="all";teacherShowDone=false;teacherFrom="";teacherTo="";
...
function teacherNotesSectionHtml(name){
  ...
  const profileCard=teacherEditing
    ...
    :`<div class="tn-card tn-profile-card">
        ...
        <div class="tn-tiles">
          <div class="tn-tile"><span>📞 联系方式</span><b>${t.contact?esc(t.contact):"—"}</b></div>
          <div class="tn-tile"><span>🎓 负责 / 角色</span><b>${t.role?esc(t.role):"—"}</b></div>
          <div class="tn-tile"><span>📌 备注</span><b>${t.note?esc(t.note):"—"}</b></div>
        </div>
      </div>`;
  // 2) 沟通笔记卡：可折叠 + 每条可改可删
  ...
  // 3) 学生对老师的反馈卡：每条一个小卡片，可点改、可删；和学生页联动（同一份 feedbacks）
...
function renderTeacherDetail(sel,since,until,rt){
  ...
  setHead("老师面板","","");
  byId("tabs").innerHTML=`<button class="btn" id="teacherBack" type="button">← 返回</button>`;
  ...
  byId("teacherBack").addEventListener("click",()=>{teacherPanelSel="";teacherEditing=false;teacherLogEditId="";teacherFbEditId="";render();});
...
/* 老师资料 + 沟通笔记的事件绑定 */
function bindTeacherNotes(teacher){
  const guard=()=>{if(adminViewEmail){showToast("正在查看他人数据，只能浏览不能修改");return false;}return true;};
  const edit=byId("tnEdit");if(edit)edit.addEventListener("click",()=>{teacherEditing=true;render();});
  const cancel=byId("tnCancel");if(cancel)cancel.addEventListener("click",()=>{teacherEditing=false;render();});
  const save=byId("tnSave");if(save)save.addEventListener("click",()=>{
    if(!guard())return;
    const t=getTeacher(teacher);
  ...
  const addLog=()=>{
    ...
    const v=inp.value.trim();if(!v){showToast("先写一句再添加");return;}
    const t=getTeacher(teacher);t.logs=t.logs||[];
    t.logs.push({id:uid("tlog"),date:dateKey(new Date()),text:v});t.updatedAt=new Date().toISOString();
    saveTeachers();showToast("已记一条沟通");render();
  };
  ...
  const addFb=()=>{
    if(!guard())return;
    const txt=byId("tnFbText");if(!txt)return;
    const v=txt.value.trim();if(!v){showToast("先写一句反馈再添加");return;}
    const t=getTeacher(teacher);t.feedbacks=t.feedbacks||[];
    t.feedbacks.push({id:uid("fb"),date:dateKey(new Date()),student:(byId("tnFbStudent")?byId("tnFbStudent").value.trim():""),text:v});
    t.updatedAt=new Date().toISOString();saveTeachers();showToast("已记一条学生反馈");render();
  };
  const fbBtn=byId("tnFbAdd");if(fbBtn)fbBtn.addEventListener("click",addFb);
  const fbInput=byId("tnFbText");if(fbInput)fbInput.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();addFb();}});
  document.querySelectorAll("[data-tn-fb-edit]").forEach(b=>b.addEventListener("click",()=>{teacherFbEditId=b.dataset.tnFbEdit;render();}));
  document.querySelectorAll("[data-tn-fb-canceledit]").forEach(b=>b.addEventListener("click",()=>{teacherFbEditId="";render();}));
  document.querySelectorAll("[data-tn-fb-save]").forEach(b=>b.addEventListener("click",()=>{
    if(!guard())return;
    const id=b.dataset.tnFbSave;const txt=byId("tnFbEditText-"+id);if(!txt)return;
    const v=txt.value.trim();if(!v){showToast("内容不能为空");return;}
    const t=getTeacher(teacher);const f=(t.feedbacks||[]).find(x=>x.id===id);
    if(f){f.text=v;f.student=(byId("tnFbEditWho-"+id)?byId("tnFbEditWho-"+id).value.trim():f.student);t.updatedAt=new Date().toISOString();saveTeachers();}
    teacherFbEditId="";showToast("已更新反馈");render();
  }));
  document.querySelectorAll("[data-tn-fb-del]").forEach(b=>b.addEventListener("click",()=>{
    if(!guard())return;
    const t=getTeacher(teacher);t.feedbacks=(t.feedbacks||[]).filter(f=>f.id!==b.dataset.tnFbDel);t.updatedAt=new Date().toISOString();
    saveTeachers();render();
  }));
}
...
function bindTeacherPanelEvents(){
  ...
  bindDateRangeCtl("tp",(f,t)=>{teacherFrom=f;teacherTo=t;rerenderKeepScroll();});
  document.querySelectorAll("[data-teacher-pick]").forEach(b=>b.addEventListener("click",()=>{teacherPanelSel=b.dataset.teacherPick;teacherEditing=false;teacherLogEditId="";teacherFbEditId="";render();}));
}
...
function importTestData(){
  ...
  // 演示老师资料 + 沟通笔记
  if(!teachersData.some(t=>t.name==="清滢老师")){
    teachersData.push(normalizeTeacher({name:"清滢老师",contact:"微信 wx_qingying",role:"CR 中文阅读 主教",note:"沟通顺畅，批改偏慢，需每周提醒",logs:[
      {id:"test-tlog-0",date:dateKey(addDays(new Date(),-2)),text:"提醒她本周有 4 份作业还没批改，她说周末补上"},
      {id:"test-tlog-1",date:dateKey(addDays(new Date(),-9)),text:"家长反馈孩子很喜欢她的课，让她保持"}
    ]}));
  }
  scheduleData=[...scheduleData,...classes];
  studentsData=[...studentsData,...profiles];
  sopData=[...sopData,...sopCards];
  saveSchedule();saveStudents();saveSop();saveTeachers();
}
